...
const ProfilePage = ({ profile, user, initialTab = 'overview' }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-10 duration-700 py-10">
      {/* Profile Header */}
      <div className="relative bg-[#080d19]/80 backdrop-blur-xl border border-white/5 rounded-[2.5rem] p-10 mb-10 overflow-hidden shadow-[0_20px_80px_rgba(0,0,0,0.6)] group">
        {/* Animated Glow */}
        <div className="absolute top-0 right-0 w-96 h-96 bg-cyan-500/10 rounded-full blur-[120px] pointer-events-none group-hover:bg-cyan-500/20 transition-all duration-1000" />
        <div className="absolute bottom-0 left-0 w-96 h-96 bg-purple-500/10 rounded-full blur-[120px] pointer-events-none group-hover:bg-purple-500/20 transition-all duration-1000" />

        <div className="relative z-10 flex flex-col lg:flex-row items-center gap-10">
          <div className="relative">
            <div className="w-32 h-32 bg-gradient-to-tr from-cyan-400 via-blue-500 to-indigo-600 rounded-[2.5rem] flex items-center justify-center text-6xl font-black text-white shadow-2xl shadow-cyan-500/40 border border-white/20 transform rotate-3 hover:rotate-0 transition-transform duration-500">
              {profile?.email?.[0]?.toUpperCase() || 'U'}
            </div>
            <div className="absolute -bottom-2 -right-2 bg-emerald-500 w-6 h-6 rounded-full border-4 border-[#080d19] shadow-[0_0_15px_rgba(16,185,129,0.8)]" />
          </div>
          <div className="text-center lg:text-left flex-1">
            <div className="flex items-center justify-center lg:justify-start gap-3 mb-2">
              <span className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.4em] bg-emerald-500/10 px-3 py-1 rounded-full border border-emerald-500/20">Active Node</span>
            </div>
            <h2 className="text-4xl md:text-5xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400 mb-2">{profile?.email?.split('@')[0]}</h2>
            <p className="text-sm font-medium text-slate-400 mb-6">{profile?.email}</p>
            <div className="flex flex-wrap justify-center lg:justify-start gap-4">
              <span className="bg-gradient-to-r from-cyan-500/20 to-blue-500/20 text-cyan-300 border border-cyan-500/30 px-5 py-2 rounded-xl text-[11px] font-bold uppercase tracking-widest flex items-center gap-2 shadow-[0_0_20px_rgba(6,182,212,0.15)]">
                <User size={14} /> {profile?.role || 'User'}
              </span>
              <span className="bg-white/5 text-slate-300 border border-white/10 px-5 py-2 rounded-xl text-[11px] font-bold tracking-widest flex items-center gap-2">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-12">
        {statCards.map((stat, idx) => (
          <motion.div
            ...
            className={`bg-[#080d19]/60 backdrop-blur-md border ${stat.border} rounded-[2rem] p-7 relative overflow-hidden group hover:-translate-y-1 transition-all duration-300 shadow-xl`}
          >
            <div className={`absolute -top-10 -right-10 w-32 h-32 bg-gradient-to-bl ${stat.color} opacity-10 group-hover:opacity-20 rounded-full transition-opacity duration-500 blur-2xl`} />
      ...
      {/* Tabs */}
      <div className="flex items-center gap-3 mb-10 overflow-x-auto no-scrollbar pb-4 px-2">
        {tabs.map(tab => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`flex items-center gap-2.5 px-6 py-3.5 rounded-2xl text-[11px] font-black uppercase tracking-[0.15em] transition-all whitespace-nowrap border ${activeTab === tab.id
              ? 'bg-cyan-500/20 border-cyan-500/50 text-cyan-300 shadow-[0_10px_30px_rgba(6,182,212,0.2)] scale-105'
              : 'bg-[#080d19]/80 backdrop-blur-md border-white/5 text-slate-400 hover:text-white hover:border-white/20 hover:bg-white/5'
              }`}
          >
            {tab.icon} {tab.label}
          </button>
