...
export default function MovieCard({ movie, onSelect, onAdd }) {
  // Hover durumunu takip etmek için state
  ...
  return (
    <motion.div
      className="relative group bg-slate-900/40 rounded-[2rem] overflow-hidden border border-slate-800 transition-all duration-300 shadow-3xl hover:border-cyan-500/50 hover:shadow-2xl hover:shadow-cyan-500/10 cursor-pointer"
          ...
          >
            <div className="space-y-6">
              ...
              <div className="space-y-2 text-[10px] text-slate-600 font-mono">
                <p><span className="text-slate-400 font-bold uppercase tracking-widest">Tür:</span> {genres}</p>
                <p><span className="text-slate-400 font-bold uppercase tracking-widest">Oyuncular:</span> {cast}</p>
              </div>
              
              <div className="flex gap-4 mt-6">
                <Share2 size={18} className="text-slate-700 hover:text-cyan-500 transition-colors" />
                <ThumbsUp size={18} className="text-slate-700 hover:text-cyan-500 transition-colors" />
                <Info size={18} className="text-slate-700 hover:text-cyan-500 transition-colors" />
              </div>
            </div>
          </motion.div>
        )}
